...
*   **Phase 2: Feature Implementation (TypeQL)**
    ...
    *   [X] Implement error handling and propagation (Server-side request handler, Client-side React hook).
    ...
    *   [X] Implement WebSocket transport logic (`createWebSocketTransport`) including connection, request/response correlation, subscription handling placeholders, reconnect logic, automatic resubscription, `onAckReceived` callback, and `requestMissingDeltas` method. **Updated iterator to yield full message.**
...
*   **Known Issues**: Conflict resolution details TBD. Server needs a way to identify clients/transports for subscriptions. `useQuery` optimistic update integration assumes TState is compatible with TOutput. Client-side error propagation beyond React hooks needs review.

...
import {
    ...
    MutationCallOptions,
    PredictedChange,
    StoreListener, // Import StoreListener type
    UnsubscribeStoreListener, // Import UnsubscribeStoreListener type
...
>(
    ...
    // --- FIX: Initialize state synchronously from store if available ---
    const getInitialData = useCallback((): TOutput | undefined => {
        if (store && enabled) { // Only read if store exists and query is enabled
            try {
                const currentState = store.getOptimisticState();
                // Ensure select is only called if it exists
                ...
                // Fallback to undefined if initial read fails
            }
        }
        return undefined;
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [store, enabled, select]); // Dependencies for initial data getter
    ...
    // Define initial status based on whether we got data from the store
    ...
    // Define isFetching state correctly
    const [isFetching, setIsFetching] = useState(enabled && data === undefined); // Fetching only if enabled and no initial data
    // Initialize fetch time ref based on initial data source
    const lastFetchTimeRef = useRef<number>(data !== undefined ? Date.now() : 0);

    // isLoading is true if status is 'loading'
    ...
    const executeQuery = useCallback(async (forceRefetch = false): Promise<void> => {
        // Prevent concurrent fetches using the state variable
        ...
        setIsFetching(true); // Use the state setter
        // Set status to 'loading' only if there's no data yet
        if (data === undefined) {
            setStatus('loading');
        }
        // Don't reset error here, keep previous error if background refetch fails
        // setError(null); // Reset error on new fetch - REMOVED

        let queryResult: TOutput | undefined;
        let queryError: TypeQLClientError | Error | null = null;

        try {
            queryResult = await procedure.query(input) as TOutput; // Direct call
        } catch (err: any) {
             console.error("[TypeQL Preact] useQuery Error:", err); // Update log prefix
             queryError = err instanceof TypeQLClientError ? err : err instanceof Error ? err : new TypeQLClientError(String(err?.message || err));
        } finally {
             // --- FIX: Group all state updates after await within a single isMounted check ---
             if (isMountedRef.current) {
                 queueMicrotask(() => {
                     if (!isMountedRef.current) return; // Re-check mount status inside microtask
                     setIsFetching(false); // Always set fetching to false in finally if mounted
                     if (queryError) {
                         setError(queryError);
                         setStatus('error');
                         ...
                         // Success case
                         setData(queryResult); // queryResult should be defined here if no error
                         setStatus('success');
        ...
        // Update dependencies for executeQuery - REMOVE state variables and setters
    }, [procedure, inputKey, staleTime, enabled]); // Keep only external dependencies

    useEffect(() => {
        // FIX: Only trigger executeQuery if enabled AND we don't have initial data from store
        if (enabled && data === undefined) {
            // Initial fetch if enabled and no initial data from store
            void executeQuery();
        } else if (!enabled) {
             // Reset state if disabled (and not already idle/success without data)
             if (status !== 'success' || data !== undefined || error !== null) {
                 setData(undefined);
                 setIsFetching(false); // Use the state setter
                 setStatus('success'); // Treat as success with no data/error
                 setError(null);
                 lastFetchTimeRef.current = 0; // Reset fetch time
             }
        }
        // Dependencies: enabled, inputKey, procedure ref, executeQuery callback - REMOVE state variables
    }, [enabled, inputKey, procedure, executeQuery]); // Only depend on things triggering a new fetch

    // Memoize the store listener function
    // Correct signature: (optimisticState: TState, confirmedState: TState) => void
    const storeListener = useCallback((optimisticState: TState, confirmedState: TState) => {
        if (isMountedRef.current) {
            // We typically want to reflect the optimistic state in the UI
            try {
                if (select) {
                ...
                // If we get an update from the store, assume it's 'success' unless an error occurs later
                // Don't reset error here, let network fetch handle that
                if (status !== 'success') setStatus('success');
                // Should we update lastFetchTimeRef here? Probably not, as it's a store update.
            } catch (e) {
                console.error("[useQuery] Error selecting state from optimistic update:", e);
                // Optionally set error state?
            }
       }
   }, [select, status, setStatus]); // Adjusted dependencies

    // Subscribe to store updates if a store is provided
    useEffect(() => {
        // FIX: Remove initial sync logic from here, it's handled by useState initializer now
    ...
    }, [store, enabled, storeListener]); // Simplify dependencies further, rely on useCallback for listener stability
    ...
    return {
        data,
        isLoading,
        isFetching, // Return the state variable
